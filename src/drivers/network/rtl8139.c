...
void Rtl8139Receive()
{
    ...
    while ((In8(private->ioAddress + RTL8139_COMMAND) & 0x01) == 0) {
        ...
        /* 写入寄存器 */
        Out16(private->ioAddress + RTL8139_CAPR, cur - 16);
    }
...
PUBLIC int InitRtl8139Driver()
{
    ...
    /* init receive buffer */
    private->rxBuffer = (unsigned char *) kmalloc(PAGE_SIZE * 4, GFP_KERNEL);
    ...
    private->currentRX = 0;
    /* 4个页长度，每个接收缓冲区4096字节 */
    private->rxBufferLen = 4 * PAGE_SIZE;
    ...
    for (i = 0; i < 4; i++) {
        private->txBuffers[i] = (uint8 *) kmalloc(PAGE_SIZE, GFP_KERNEL);
