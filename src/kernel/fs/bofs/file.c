...
PUBLIC void BOFS_InitFdTable()
{
	...
	while (fdIdx < BOFS_MAX_FD_NR) {
        ...
        BOFS_GlobalFdTable[fdIdx].pipe = NULL;
...
{
	file->flags = BOFS_FD_FREE;
    file->inode = NULL;
    file->inode = NULL;
    file->dirEntry = NULL;
    file->parentEntry = NULL;
    file->pos = 0;
	file->superBlock = NULL;
}

PUBLIC void BOFS_FileDescriptorInit(int fd)
{
    if(fd < 0 || fd >= BOFS_MAX_FD_NR) {
		printk("fd error\n");
		return;
...
PUBLIC int FdLocal2Global(uint32_t localFD)
{
    ...
    int globalFD = cur->fdTable[localFD]; /* 文件描述符表中存放的就是全局的文件描述符 */
    return globalFD;
}
...
/**
 * BOFS_IsFileOpened - 检查文件是否已经打开
 * @fd: 文件描述符
 * 
 * @return: 是打开返回1，否则返回0
 */
PUBLIC int BOFS_IsFileOpened(int fd)
{
    if (fd < 0 || fd >= MAX_OPEN_FILES_IN_PROC) {
		printk("bofs: fd error\n");
		return -1;
	}
	...
	struct BOFS_FileDescriptor *fdptr = BOFS_GetFileByFD(globalFD);
    if (fdptr == NULL) {
        return 0;
    }
    if (fdptr->flags & BOFS_FD_USING) {
        return 1;
    }
	...
	struct BOFS_SuperBlock *sb)
{
	...
	*/
	...
	if(result > 0){
        ...
        AtomicSet(&BOFS_GlobalFdTable[fd].reference, 1);
		
		BOFS_GlobalFdTable[fd].flags |= flags;
...
PUBLIC int BOFS_Trancate(int fd)
{
    ...
    /* 把节点数据都清除 */
    BOFS_ReleaseInodeData(fdptr->superBlock, fdptr->inode);
    
    /* 大小置0 */
    fdptr->inode->size = 0;
    
    /* 修改了大小，需要同步回磁盘 */
    BOFS_SyncInode(fdptr->inode, fdptr->superBlock);
 ...
 * @fd: 旧文件描述符
 * 
 * @return: 返回新的描述符，失败返回-1
 */
PUBLIC int BOFS_Dup(int oldfd)
{
    if (oldfd < 0 || oldfd >= MAX_OPEN_FILES_IN_PROC) {
		printk("bofs dup: fd error\n");
		return -1;
	}
    /* old  must open */
...
PUBLIC long BOFS_IsFoot(int fd)
{
    ...
    /* 如果位置是在文件末尾，就返回1 */
    return fdptr->pos >= fdptr->inode->size ? 1 : 0;
}
...
PUBLIC int BOFS_Open(const char *pathname, unsigned int flags, struct BOFS_SuperBlock *sb)
{
	...
	*/
    ...
    char extend = 0;
    /* 只能是截断或者追加其中之一 */
    if(flags & BOFS_O_TRUNC){
		extend = 1; /* 截断 */
	} else if(flags & BOFS_O_APPEDN){
		extend = 2; /* 追加 */
	}
	...
	if((flags & BOFS_O_RDONLY) || (flags & BOFS_O_WRONLY) || (flags & BOFS_O_RDWR)){
        ...
        if (record.childDir->type == BOFS_FILE_TYPE_NORMAL || 
            ...
            /* 打开文件描述符 */
            fd = BOFS_OpenFile(record.parentDir, name, mode, flags, record.superBlock);
            ...
            } else if (record.childDir->type == BOFS_FILE_TYPE_FIFO) {
...
PUBLIC int BOFS_Write(int fd, void *buf, unsigned int count)
{
    ...
    if (BOFS_IsPipe(fd)) {
        ...
        if(wrFile->flags & BOFS_O_WRONLY || wrFile->flags & BOFS_O_RDWR){
            ...
            printk("task %s pid %d\n", CurrentTask()->name, CurrentTask()->pid);
        }
    }
	return ret;
}
        ...
        if(rdFile->flags & BOFS_O_RDONLY || rdFile->flags & BOFS_O_RDWR){
            ...
            if(rdFile->inode->mode & BOFS_IMODE_R){
                ...
                if (rdFile->dirEntry->type == BOFS_FILE_TYPE_NORMAL) {
                    ...
                    //printk("fifo read!\n");
                    ret = BOFS_FifoRead(rdFile, buf, count);
...
PUBLIC void BOFS_UpdateInodeOpenCounts(struct Task *task)
{
    ...
    struct BOFS_FileDescriptor *file;
